/* main.c — Zchezz v3.13 UCI engine
 *
 ...
 *
 * CRITICAL INVARIANTS:
 *   1. board_make() calls nnue_push() which uses GLOBAL _acc_ptr/_acc_buf.
 *      This is NOT thread-safe — each helper thread's push/pop would
 *      collide with the main thread's.  Currently this works because
 *      the NNUE globals (_acc_ptr, _acc_buf_w, _acc_buf_b) are written
 *      by ALL threads concurrently.  This produces benign data races
 *      (wrong accumulator values) which are tolerable in Lazy SMP
 *      because each thread's search is independent.
 *      TODO: make _acc_ptr/_acc_buf per-thread via NnueAccum for true
 *      thread safety.  Current approach works but is technically UB.
 *
 *   2. TT_GEN is incremented by main thread only.  Helpers read but
 *      don't write (no TT aging conflicts).
 *
 *   3. Helper's Board is a COPY made at launch time.  board_make/unmake
 *      operate on the copy, so the main thread's board is never touched.
 *
 *   4. Root TB filtering only runs when start_depth <= 1 (main thread).
...
static void *helper_thread_fn(void *arg) {
     ...
     *
     * v3.13: NnueAccum is heap-allocated per thread (~4.3 KB),
     * same pattern as the undo stack. */
    UndoFrame *my_undo = (UndoFrame *)malloc(STACK_SIZE * sizeof(UndoFrame));
    int my_undo_top = 0;
    ha->board.undo = my_undo;
    ha->board.undo_top = &my_undo_top;

    /* Per-thread NNUE accumulator (v3.13) */
