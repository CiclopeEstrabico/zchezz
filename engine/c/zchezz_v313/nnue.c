...
/* nnue.c — Zchezz NNUE evaluation layer (v3.13: thread-safe accumulators)
 *
 * v3.13 CHANGE: All per-evaluation mutable state (accumulators, ext cache)
 * is passed via NnueAccum* instead of global statics.  This makes
 * nnue_eval/nnue_eval_bb thread-safe for Lazy SMP.
 * Weight tables remain global (read-only after load, safe for MT).
 * Legacy nnue_push/nnue_pop still use old global arrays for test functions.
 *
...
void nnue_reset(NnueAccum *na) {
    ...
    /* Legacy globals (for push/pop test API) */
    _acc_ptr   = 0;
    _acc_dirty = 1;
...
void nnue_rebuild(NnueAccum *na, const uint8_t *board) {
    ...
    na->acc_dirty = 0;
    /* Sync legacy globals for push/pop tests */
    _acc_dirty = 0;
    _acc_ptr   = 0;
    memcpy(_acc_buf_w[0], dW, NN_L1_OUT*sizeof(int16_t));
    memcpy(_acc_buf_b[0], dB, NN_L1_OUT*sizeof(int16_t));
...
/* ── Push / Pop (legacy — operates on global arrays, NOT NnueAccum) ── */
/* Retained for NNUE unit tests.  Search does NOT call these.
 * Uses the old global _acc_buf_w/b stacks, not the NnueAccum struct.
 * If _acc_dirty is set, calls nnue_rebuild on g_nnue_accum to seed slot [0]. */
void nnue_push(const uint8_t *board, const NNMove *m) {
    int src=_acc_ptr, dst=src+1;
    if (dst>=NN_ACC_DEPTH) { _acc_dirty=1; return; }
    if (_acc_dirty) { nnue_rebuild(&g_nnue_accum, board); _acc_dirty=0; }
                 ...
                 const uint64_t bb[12], uint64_t board_hash)
{
     ...
     *
     * Read HM accumulator from legacy globals _acc_buf_w/b[_acc_ptr].
     * board_make() calls nnue_push() which increments _acc_ptr and writes
     * the incrementally-updated accumulator for the new position.
     * board_unmake() calls nnue_pop() which decrements _acc_ptr.
     *
     * CRITICAL: push/pop MUST be called from board_make/unmake.
     * Without them, _acc_ptr stays at 0 (root), the HM features never
     * change, and ALL positions evaluate to the same constant (±42cp).
     * This was the v3.13 regression bug — verified by removing push/pop
     * and observing identical evals at every search node. */
    uint8_t relu1[NN_L1_OUT] __attribute__((aligned(32)));
    const int16_t *acc = stm==0 ? _acc_buf_w[_acc_ptr] : _acc_buf_b[_acc_ptr];
