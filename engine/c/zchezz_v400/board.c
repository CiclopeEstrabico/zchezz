...
/* Default global NNUE accumulator.
 * Used by main thread — SMP helpers allocate their own on the heap.
 * Must be 32-byte aligned for AVX2 SIMD in nnue_eval.
 *
 * CRITICAL: acc_dirty MUST start as 1 so that the forward pass returns
 * 0 until nnue_rebuild is explicitly called.  Zero-initialization would
 * leave acc_dirty=0, causing the forward pass to run on a zero
 * accumulator (garbage evaluations — this was the v3.13 regression).
 *
 * v4.00: ~257 KB (the two acc stacks are 512 wide now).  The former
 * .ext_dirty seed is gone with the extra features. */
...
void board_make(Board *b, const Move *m) {
     ...
     * Without push/pop, acc_ptr stays at 0 (root position), the
     * accumulator never changes, and every node evaluates to the same
     * score — the engine plays random moves.
     *
     * v4.00: the accumulator is HalfKP-4Bucket, so the feature index of
     * EVERY piece depends on its own side's king bucket.  nnue_push_na
     * therefore needs both king squares as they are BEFORE the move, to
     * (a) index the pieces it adds/removes and (b) detect a king move
     * that crosses a bucket border (which invalidates that whole
     * perspective — see the "Bucket refresh" section of nnue.h).
     * Passing b->wk / b->bk here is what lets nnue_push_na stay free of
     * the 64-square king scan it would otherwise need on every node. */
    if(nnue_ready() && b->nnue){
        NNMove nm; nm.from_sq=(uint8_t)f; nm.to_sq=(uint8_t)to;
        nm.prom=m->prom; nm.is_epc=m->epc; nm.castle=m->castle;
        nm.wk_sq=b->wk; nm.bk_sq=b->bk;   /* v4.00: pre-move king squares */
