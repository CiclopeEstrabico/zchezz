/* board.c — Zchezz board: 64-bit BBs + magic bitboards
 *
...
void board_make(Board *b, const Move *m) {
    if(g_undo_top>=STACK_SIZE){fprintf(stderr,"[BUG] undo overflow\n");return;}
    UndoFrame *uf=&g_undo[g_undo_top++];
    ...
    b->hist_len++;
    if(g_hist_len<HIST_SIZE){g_hist[g_hist_len]=b->hash;}
    g_hist_len++;
...
void board_unmake(Board *b) {
    if(nnue_ready())nnue_pop();
    b->hist_len--; g_hist_len--;
    UndoFrame *uf=&g_undo[--g_undo_top];
...
int board_is_draw(const Board *b) {
    ...
    if(b->hm<4||g_hist_len<2)return 0;
    uint64_t h=b->hash;
    int reps=1;
    int limit=g_hist_len-(b->hm+2<100?b->hm+2:100);
    if(limit<0)limit=0;
    for(int i=g_hist_len-2;i>=limit;i-=2)
        if(g_hist[i]==h){if(++reps>=3)return 2;}
    if(reps==2)return 3;
    return 0;
}
