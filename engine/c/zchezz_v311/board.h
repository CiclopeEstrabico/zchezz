...
typedef struct {
    ...
    int      hist_len;
    /* Per-board undo stack — each Board copy has its own stack so
     * Lazy SMP helper threads don't corrupt the main thread's state.
     * In single-thread mode this is the same Board instance, zero overhead. */
    UndoFrame undo[STACK_SIZE];
    int       undo_top;
} Board;

/* Legacy globals — kept for backward compat but no longer used by board_make/unmake */
