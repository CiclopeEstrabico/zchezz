...
/* nnue.c — Zchezz NNUE evaluation layer (OPTIMISED QAT int16/int8)
 *
...
static int8_t  _ext_dirty[NN_ACC_DEPTH][2];

/* Stored feature values per stack slot, per stm direction.
 * Allows incremental updates: only re-project features that changed. */
static float   _ext_feat[NN_ACC_DEPTH][2][NN_EXTRA];

/* ── 2-slot extra-feature cache for nnue_eval_bb ─────────────────── */
#define EXT_CACHE_SLOTS 4
static uint64_t _ext_cache_key[EXT_CACHE_SLOTS];
static int16_t  _ext_cache_buf[EXT_CACHE_SLOTS][NN_L1_OUT] __attribute__((aligned(32)));

void nnue_reset(void) {
    _acc_ptr   = 0;
    _acc_dirty = 1;
    memset(_ext_dirty, 1, sizeof(_ext_dirty));
    memset(_ext_cache_key, 0, sizeof(_ext_cache_key));  /* Clear ALL slots */
}
...
void nnue_rebuild(const uint8_t *board) {
    int16_t *dW = _acc_buf_w[0], *dB = _acc_buf_b[0];
    ...
    _acc_dirty = 0;
    _acc_ptr   = 0;
    /* Seed feature arrays and project for both stm directions */
    for (int stm=0; stm<2; stm++) {
        _compute_extra_feat(_ext_feat[0][stm], board, stm);
        memset(_ext_buf[0][stm], 0, NN_L1_OUT*sizeof(int16_t));
        _project_feat_full(_ext_buf[0][stm], _ext_feat[0][stm]);
        _ext_dirty[0][stm] = 0;
    }
}
...
void nnue_push(const uint8_t *board, const NNMove *m) {
    ...
    if (_acc_dirty) { nnue_rebuild(board); _acc_dirty=0; }
...
int nnue_eval(int stm, const uint8_t *board) {
    ...
    if (_acc_dirty)   return 0;

    /* Steps 1+2: extra features → ext_buf, then acc + ext + bias → relu1.
     *
     * WHY always recompute: search.c uses board_make/unmake but does NOT call
     * nnue_push/pop, so _acc_ptr stays 0 throughout search.  Caching ext_buf
     * would freeze it at the ROOT board's piece counts/pawn structure for the
     * entire ID iteration, causing 300–600 cp errors in endgames (KQK vs KRK
     * returning identical scores; confirmed 533 cp error).
     *
     * PERFORMANCE: we use the fast int16 _project_feat_full path (31 × 16-wide
     * AVX2 mullo_epi16 loops) to build ext_buf, then the unchanged step-2 kernel
     * adds acc + ext + bias.  Total extra cost vs the broken cached version is
     * exactly the _project_feat_full call (~31 × 16 SIMD iters = ~496 instructions).
     * This is acceptable: ~15-20% slower than the buggy version, but correct. */

    /* Step 1: compute extra feature values and project into ext_buf */
    {
        int ptr = _acc_ptr;
        _compute_extra_feat(_ext_feat[ptr][stm], board, stm);
        memset(_ext_buf[ptr][stm], 0, NN_L1_OUT*sizeof(int16_t));
        _project_feat_full(_ext_buf[ptr][stm], _ext_feat[ptr][stm]);
        /* _ext_dirty not used for caching anymore — kept for nnue_push incremental path */
    }

    /* Step 2: ClippedReLU → relu1[256] uint8 */
    uint8_t relu1[NN_L1_OUT] __attribute__((aligned(32)));
    const int16_t *acc = stm==0 ? _acc_buf_w[_acc_ptr] : _acc_buf_b[_acc_ptr];
    const int16_t *ext = _ext_buf[_acc_ptr][stm];
...
/* 2-slot direct-mapped ext cache.
 * Slot index = board_hash_key & 1.
 * On hit: ext_buf already projected; just use it.
 * On miss: compute feat + project, store in slot. */
...
int main(int argc, char **argv) {
    ...
    int score_after=nnue_eval(1,board_e4);
    printf("[NNUE TEST] After 1.e4 stm=B: %d\n", score_after);
    nnue_pop();
    int score_back=nnue_eval(0,board);
    ...
    long t0=now_ns();
    volatile int sum=0;
    for(int i=0;i<N;i++) sum+=nnue_eval(i&1,board);
    long t1=now_ns();
    double ms=(t1-t0)/1e6;
     ...
     * This is closer to real alpha-beta: eval(d0), push, eval(d1), push, ...
     * The incremental optimization shines here because each eval uses the
     * parent's clean ext_buf from the previous eval. */
    int S=200000;
    long t4=now_ns();
    volatile int sum3=0;
    nnue_rebuild(boards[0]);
    for (int i=0; i<S; i++) {
        sum3 += nnue_eval(0, boards[0]);
        for (int d=0; d<4; d++) {
            nnue_push(boards[d], &moves[d]);
            sum3 += nnue_eval((d+1)&1, boards[d+1]);
        }
        for (int d=0; d<4; d++) nnue_pop();
    }
    long t5=now_ns();
