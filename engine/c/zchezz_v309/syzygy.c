...
int syzygy_probe_wdl(const Board *b, int *wdl) {
    ...
    /* Check piece count: skip positions with < 4 pieces.
     * KvK (2) and KXK (3) endgames crash the Fathom library on some
     * TB file sets, and the engine evaluates them correctly without TB. */
    int npieces = __builtin_popcountll(b->occ);
    if (npieces < 4) return 0;
    ...
    unsigned result = tb_probe_wdl(
        ...
        0,                /* rule50: always 0 for WDL (Fathom rejects non-zero) */
