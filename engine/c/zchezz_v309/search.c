...
static int alpha_beta(SearchState *ss, Board *b, int depth, int alpha, int beta,
    ...
    *pv_len = 0;
    ...
    int cmh0 = ply >= 1 ? ss->prev_to_sq[ply-1] : -1;   /* 1-ply CMH context */
    int cmh1 = ply >= 2 ? ss->prev_to_sq[ply-2] : -1;   /* 2-ply CMH context */
    ...
    for (int i = 0; i < n; i++)
        moves[i].score = score_move(ss, &moves[i], b, ply, pv_ptr, ok_sq,
