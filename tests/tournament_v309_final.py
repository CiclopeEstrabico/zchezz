#!/usr/bin/env python3
"""Final validation: v309 (no TB, 1 thread) vs v306 — 600 games, 15 workers."""
...
def main():
    ...
    b_cfg = {
        ...
        "options": {"NNUE": os.path.join(BASE, "engine", "c", "zchezz_v306", "nnue_weights.bin")},
    }
    ...
    print(f"  FINAL VALIDATION: v309 (no TB) vs v306 (baseline)")
    print(f"{'=' * 60}")
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        ...
        for f in as_completed(futures):
            ...
            if game_num % 10 == 0 or game_num <= 5 or game_num == len(games):
