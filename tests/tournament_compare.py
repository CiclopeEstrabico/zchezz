...
def compute_elo(results, anchors_used):
    """Given list of (score, anchor_elo), compute engine ELO via elo_calc."""
    if not results:
        return 0, 0
    # Group by anchor ELO
    per_anchor = {}
    for score, anch_elo in results:
        if anch_elo not in per_anchor:
            per_anchor[anch_elo] = {"w": 0, "d": 0, "l": 0}
        if score == 1.0: per_anchor[anch_elo]["w"] += 1
        elif score == 0.0: per_anchor[anch_elo]["l"] += 1
        else: per_anchor[anch_elo]["d"] += 1

    weighted_elo = 0.0
    combined_var = 0.0
    for a_elo, s in per_anchor.items():
        est, ci = estimated_elo(s["w"], s["d"], s["l"], a_elo)
    ...
    if combined_var > 0:
        ...
        return round(final_elo, 1), round(final_ci, 1)
    return 0, 999
