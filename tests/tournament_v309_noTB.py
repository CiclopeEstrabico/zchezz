#!/usr/bin/env python3
"""Test 1: v309 (no TB) vs v308 (no TB) — verify no regression from TB code changes."""
...
def main():
    ...
    # TEST 1: v309 (no TB) vs v308 (no TB) — pure code regression check
    a_cfg = {
        "name": "v309",
        "path": os.path.join(BASE, "engine", "c", "zchezz_v309", "zchezz.exe"),
        "options": {"NNUE": os.path.join(BASE, "engine", "c", "zchezz_v309", "nnue_weights.bin")},
    }
    b_cfg = {
        "name": "v308",
        "path": os.path.join(BASE, "engine", "c", "zchezz_v308", "zchezz.exe"),
        "options": {"NNUE": os.path.join(BASE, "engine", "c", "zchezz_v308", "nnue_weights.bin")},
    }

    a_name, b_name = a_cfg["name"], b_cfg["name"]
    print(f"\n{'=' * 60}")
    print(f"  {a_name} (no TB) vs {b_name} (no TB) ({GAMES} games)")
    print(f"  Movetime: {MOVETIME}ms | Workers: {WORKERS}")
    print(f"  Phase 4 Test 1: No regression from Stockfish-style TB code")
    print(f"{'=' * 60}")
