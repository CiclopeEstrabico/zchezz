#!/usr/bin/env python3
"""200-game H2H with proper opening pairs (same opening, reversed colors)."""
...
def run_match(a_cfg, b_cfg, label=""):
    a_name = a_cfg["name"]
    b_name = b_cfg["name"]

    print(f"\n{'=' * 60}")
    print(f"  {label or f'{a_name} vs {b_name}'}")
    print(f"  {GAMES} games (100 openings x 2 colors)")
    print(f"  Movetime: {MOVETIME}ms | Workers: {WORKERS}")
    print(f"{'=' * 60}")

    # Build game pairs: each opening played with both color assignments
    ...
    for i in range(n_openings):
        ...
        # Game 2: b=white, a=black (same opening, reversed colors)
...
def main():
    ...
    NNUE_306 = os.path.join(BASE, "engine", "c", "zchezz_v306", "nnue_weights.bin")
    NNUE_215 = os.path.join(BASE, "engine", "old", "c", "zchezz_v215", "nnue_weights.bin")

    v306 = {
        "name": "v306",
        "path": os.path.join(BASE, "engine", "c", "zchezz_v306", "zchezz.exe"),
        "options": {"NNUE": NNUE_306},
    }
    v215 = {
        "name": "v215",
    ...
    # Phase 1: v306 (no TB) vs v215 — must achieve parity
    r1 = run_match(v306, v215, "Phase 1: v306 (no TB) vs v215 — parity check")
    ...
    n = r1["w"] + r1["d"] + r1["l"]
    s = r1["w"] + r1["d"] * 0.5
    print(f"  {r1['a']:10s} vs {r1['b']:10s}: +{r1['w']} ={r1['d']} -{r1['l']} "
          f"({s:.1f}/{n} = {s/n*100:.1f}%) -> {r1['elo']:+.0f} ELO")
