"""
endgame_generator.py  (v3)
──────────────────────────
Generates synthetic endgame positions, evaluates them with Stockfish,
and saves parquet files in the same schema as sf_analyze_all_quietfilter2.py
(columns: fen, cp, wdl, result, id).

Changes vs v2:
  BUG FIX 1 — is_check() → was_into_check()
    board.is_check() tells you whether the side-TO-MOVE is in check, which
    is perfectly legal.  The correct rejection is: did the previous "move"
    leave the non-moving side's king in check?  was_into_check() does that.
    The old code silently discarded a large fraction of legal positions.

  BUG FIX 2 — mirror() inherits castling rights
    chess.Board.mirror() re-parses from FEN so castling rights that were
    cleared on the source board can be re-set on the mirror.  The comment
    in v2 described the fix but it was already applied; no change needed.

  QUIET FILTER (new)
    Positions are now passed through a quiet filter (same logic as
    sf_analyze_all_quietfilter.py) before Stockfish evaluation:
      - reject if side-to-move is in check
      - reject if a winning capture is available (victim > attacker + 50cp)
      - reject if an equal capture is available (|diff| ≤ 50cp), pxp exempt
    Applied in the worker before SF to avoid wasting engine time.

  RANDOMIZED ANALYSIS WITH PROGRESS SAVE
    generate_all_records() now interleaves groups round-robin before the
    global shuffle, so a partial run already has balanced group coverage.
    Per-group saved counts are written to progress.json at every chunk.
    
  NEW GROUPS — King + 3 Pawns vs King + 3 Pawns
    ([P,P,P], [P,P,P]) added as "KPPPKppp".
"""
...
OUT_DIR   = r"data\endgame_synthetic2"
...
# ── WDL curve ──────────────────────────────────────────────────────────────
# cp(mate_dist) = max(floor, CP_MAX_MATE * exp(-K_DECAY * (dist-1)))
# wdl = 1 / (1 + exp(-cp / WDL_DIVISOR))
# Calibrated to:
#   Mate  1 -> wdl ~0.9999
#   Mate 10 -> wdl ~0.9988
#   Mate 30 -> wdl ~0.9670
...
def is_quiet(board: chess.Board) -> Tuple[bool, str]:
    """
    Returns (True, "") if position is quiet; (False, reason) otherwise.
    Mirrors the logic in sf_analyze_all_quietfilter.py:
      - Reject if in check (side to move)
      - Reject if any winning capture available (victim value > attacker + tolerance)
      - Reject if any equal capture available (|diff| <= tolerance), pawn x pawn exempt
    """
    if board.is_check():
...
def _worker_evaluate(records: List[Dict]) -> Tuple[List[Dict], Dict[str, int]]:
    ...
    try:
        for rec in records:
            ...
            # White-POV cp (same convention as main pipeline)
            side         = fen.split()[1]
            ...
            if   cp_white >  50: result = "1-0"
            elif cp_white < -50: result = "0-1"
            else:                result = "1/2-1/2"
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=N_WORKERS) as pool:
        ...
        while not exhausted or futures:
            ...
            for fut in done_futs:
                ...
                while len(pending_save) >= CHUNK_SIZE_SAVE:
                    ...
                    pd.DataFrame(save_slice).to_parquet(path, index=False)
        ...
        if pending_save:
            path = os.path.join(OUT_DIR, f"chunk_{chunk_idx:04d}.parquet")
            pd.DataFrame(pending_save).to_parquet(path, index=False)
