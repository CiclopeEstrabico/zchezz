...
feat[780] = 78.0 / 78.0
feat[781] = 1.0
feat[782:790] = 0.0 # file pawns
feat[790:798] = 0.0 # file pawns
feat[798] = 1.0 # king distance

hm = np.dot(L1W[:, :768], feat[:768]) * 255
ext = np.dot(L1W[:, 768:], feat[768:]) * 255
bias = L1B * 255

print(f"Python HM sum: {np.sum(np.round(hm))}")
print(f"Python Ext sum: {np.sum(np.round(ext))}")
print(f"Python Bias sum: {np.sum(np.round(bias))}")
